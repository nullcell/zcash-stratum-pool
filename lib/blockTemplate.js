...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    // https://github.com/zcash/zcash/src/primitives/block.h#L43
    this.serializeHeader = function(merkleRoot, nTime, nonce, equihashSolution){

        var header =  new Buffer(80);
        var position = 0;
        header.write(equihashSolution, position, equihashSolution.length, 'hex');
        header.write(nonce, position += equihashSolution.length, 4, 'hex');
        header.write(rpcData.bits, position += 4, 4, 'hex');
        header.write(nTime, position += 4, 4, 'hex');
        header.write(hashReserved, position += 4, 32, 'hex');
